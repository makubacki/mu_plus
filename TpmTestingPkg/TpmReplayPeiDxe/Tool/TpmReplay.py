...
def _begin() -> int:
    ...
    input_file_group.add_argument(
        ...
        help="An input YAML file that contains the TCG "
        "event\nlog information to process.\n\n",
    )
    ...
    if args.input_yaml_file:
        logging.info(f"Reading input YAML file {args.input_yaml_file}")

        with open(args.input_yaml_file, "r") as y:
            yaml_data = yaml.safe_load(y)

        start_time = timeit.default_timer()
        jsonschema.validate(instance=yaml_data, schema=json_schema)
        end_time = timeit.default_timer() - start_time
        logging.debug(f"{PERF} JSON schema validation took {end_time:.2f} seconds.")

        start_time = timeit.default_timer()
        log = _build_tpm_replay_event_log_from_yaml(yaml_data)
        end_time = timeit.default_timer() - start_time
        logging.debug(
