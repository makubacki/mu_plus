  ...
  )
{
  ...
  // If this is not needed on a platform, tne null instance of MmUnblockMemoryLib can be used.
  //
  ...
  )
{
  ...
  if (LoggerInfo != NULL) {
    ...
    // When PcdAdvancedLoggerBase is non-zero, the buffer is at a fixed address and is not migrated.
    //
